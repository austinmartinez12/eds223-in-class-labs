...
```{r}
#create st_point with lat long for londan
londan_point <- st_point(c(0.1,51.5))

# add a crs
londan_geom <- st_sfc(londan_point, crs = 4326)
...
```{r}
# create data frmae of atributes about londan
londan_atribute <- data.frame(
  name = "Londan",
  temperature = 25,
...
# combine geometry and DF
londan_sf <- st_sf(londan_atribute, geometry = londan_geom)
...
world_df <- world |>
  select(-geom) # dosent acctuially remove geom column
...
world_df <- world |>
  st_drop_geometry(-geom) # dosent acctuially remove geom column
...
```{r}
londan_proj = data.frame(x= 530000, y=18000) |>
...
st_crs(londan_proj) == st_crs(londan_sf)
...
```{r}
# Change the crs
londan_sf_transfrom <- st_transform(londan_sf, crs = st_crs(londan_proj))
...
if (st_crs(londan_sf_transfrom) == st_crs(londan_proj)) {
  print("its a match")
} else{
...
```{r}
st_crs(londan_sf)$IsGeographic

st_crs(londan_sf)$proj4string
```
...
```{r}
world_trasnformed <- st_transform(world, crs = 8857)

transformed_crs <- tm_shape(world_trasnformed) +
  tm_fill(fill = "area_km2")
...
density <- world |>
  ...
  summarise(population = sum(pop, na.rm =TRUE ),
            area_km2 = sum(area_km2, na.rm =TRUE ),
            "population_density" = sum(population/area_km2)) |>
  st_drop_geometry()
