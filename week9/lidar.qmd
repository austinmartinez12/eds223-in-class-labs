...
```{r}
# crop surface change to extent of the perimeter
...
# remove values outside of lowrer and upper quartile
...
tm_shape(diff_dsm_scaled) +
  ...
  tm_title("eaton pre elevation change 2025-2016") +
  tm_credits("data: open topograohy," ,
...
#crate map with hilllshade
elev_mat |>
  #create balack white hillshade efffect
  sphere_shade(texture = "bw", sunangle = 315) |>
  add_overlay(overlay, alphalayer = 0.7) |>
  #add realistic shadowws at 50% intensity ti enhance the 3D effect
...
damage_analysis <- damage_buffers |>
  mutate(lidar_min_change = lidar_min[[2]],
         elevaion_loss = abs(lidar_min_change))

```


```{r}
# Create a plot of elevation loss based on your built
...
ggplot(damage_plot,
       aes(x = age_category, y = elevaion_loss, fill = age_category)) +
