...
```{r}
#
#subset nz_hoehgts to just the features that interzect canterbury
c_hieght1 <- nz_height[canterbury, ]
```
...
#check thatv 9ntersect that intersect is opasite of disjoint
nrow(c_hieght1) + nrow(c_height_disjoint) == nrow(nz_height)
```
...
#subbset to the features in canterbury
c_height2 <- nz_height |>
...
#filter based on logical vectpr
...
#filter based on logical vecotor
...
```{r}
# agreggate values in single ooints in cycle_hire_join
...
```{r}
# Select just the value we wasnt to agregate

incongruent_2 <- incongruent |>
  select(value)

#use area wierhgted interpolation to argregate the "value" atribute

aggregating_zones_area_wieghted <- st_interpolate_aw(x=incongruent_2, 
                                                     aggregating_zones,
                                                     extensive = TRUE)

aggregating_zones_area_wieghted$value
```
...
#summerize the total popultation in each region

regions1 <- us_states |>
  group_by(REGION) |>
  summarise(population = sum(total_pop_15, na.rm = TRUE))

# alternative approuch : agregate
regions2 <- aggregate(x=us_states[, "total_pop_15"], #data to be agreggated
...
```{r}
#combine th egeometreues of western states
us_west <- us_states[us_states$REGION == "West",]
us_west_union <- st_union(us_west)

#combine geometreies of texas and western states
...
```{r}
siene_union <- st_union(seine_buffer_50km)


```

```{r}
tm_shape(siene_union) +
  tm_polygons() +
  tm_shape(seine) +
  tm_lines() +
  tm_title(text = "50km buffer")
...
```{r}
seine_simple <- st_simplify(seine, dTolerance = 11000)
