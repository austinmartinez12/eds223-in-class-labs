...
map1 <- tm_shape(zion_elevation) +
  tm_raster(col.legend = tm_legend("Evelvation(m)"))
...
#merge austria and switczerlamn rasters
...
#Assign elevatiion less than 20 an NA
...
```{r}
#create a point within arrea covered by raster
point <- matrix(c(-113, 37.5), ncol = 2)

#approuch 1 : find the cell ID for point
...
#create raster with cmaller extent
...
# verify that output has fewer values than priginal
if(ncell(zion_elevation == nrow(zion_elevation_clip))) {
  warning("clip didnt remove cells")
} else{
  print("clippojng removed cells")
}
...
# change reclassified values into factors. we making a factor for each group
values(reclassified) <- as.factor(values(reclassified))

summary(reclassified) #number of pixels in each group
```
...
ndvi_fun <- function(nir, red){ # 2 layers in using
  (nir - red) / (nir + red)
}
...
```{r}
map1 <- tm_shape(zion_elevation) +
  tm_raster(col.legend = tm_legend(title = "Elevation")) +
  tm_title(text = "Original")
