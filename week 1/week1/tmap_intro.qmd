...
world_elevation <- read_stars(here::here('week 1', 'week1', 'data', 'worldelevation.tif'))

# country boundaries
world_vector <- read_sf(here::here('week 1', 'week1', 'data', 'worldvector.gpkg'))

# major cities
world_cities <- read_sf(here::here('week 1', 'week1', 'data', 'worldcities.gpkg'))
...
tm_shape(world_vector) + #defines spacial object to plot
...
```{r}
#plotting vector and raster spacial objects
tm_shape(world_elevation)+ #plot elevations
...
```{r}
tm_shape(world_cities)+ # first tm data sets spacial extent
    tm_dots() +
    tm_text("name") +
...
tm_shape(world_vector) +
  tm_borders() +
tm_shape(world_cities, is.main = TRUE) + #plotting world_cities last, but set as main shape. sets spacial extent
  tm_dots() +
  tm_text("name")
...
tm_shape(world_elevation, bbox = c(-15, 35, 45, 65)) + # add bounding box to restict extent of map 
  tm_raster() +
tm_shape(world_vector) +
  tm_borders()
...
tm_shape(world_vector) +
  tm_polygons(fill = "life_expectancy",
              fill.scale = tm_scale(value = viridis(8)),
              fill.legend = tm_legend(title = "Life Expectancy (years)"))
...
```{r}
tmap <- tm_shape(world_vector) +
  tm_polygons(fill = "gdp_per_cap",
              fill.legend = tm_legend(title = "GDP per Capita"))

tmap_save(tmap, here::here( "week 1", "tmapmap.png"))
```
