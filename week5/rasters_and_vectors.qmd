...
original spaces around shape
cropped makes shape touch edges of map and takes away extra space
mask makes all values NA
...
```{r}
# See all elevation values belwo 2000 to NA
elevation_mask <- elevation_final
elevation_mask[elevation_mask < 2000] <- NA


# COnvert high elevation pizels into polygon shapes and then transfrom into a sf
...
# Transform the spacial data structure of raster to a matrix
...
bryce_small %>% 
  # map colors to elevation calues
  height_shade() %>%  
  #plotting the map
  plot_map() 
```
...
```{r}
# dwfine bounding box corrdinates for our area of interest
...
```{r}
#create extent object from bounding box corrdinates
...
# queryin osm for all "hoghway" features (roads, paths, trails)
...
```{r}
# sperate our lines into trails, footpaths, and roads
...
```{r}
# creating a combined trails layer with diffrrent styles for each type
